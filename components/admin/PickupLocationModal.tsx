...
export const PickupLocationModal: React.FC<PickupLocationModalProps> = ({ isOpen, onClose, onConfirm, orderId }) => {
  const [selectedLocation, setSelectedLocation] = useState<string>('');

  useEffect(() => {
    if (isOpen && PICKUP_LOCATIONS.length > 0) {
      setSelectedLocation(PICKUP_LOCATIONS[0]); // Default to first location
    } else if (isOpen) {
  ...
  const handleConfirm = () => {
    if (!selectedLocation && PICKUP_LOCATIONS.length > 0) {
      alert('Vui lòng chọn một vị trí để đồ.');
      return;
    }
    if (PICKUP_LOCATIONS.length === 0 && !selectedLocation){
      ...
      >
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <Button 
            ...
            disabled={PICKUP_LOCATIONS.length > 0 && !selectedLocation}
          >
            Xác nhận Vị trí
          </Button>
        </div>
