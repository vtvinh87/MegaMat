...
import { Order, Customer, User, ServiceRating, StaffRating, Tip, StaffRoleInOrder } from '../../types';
import { Card } from '../ui/Card';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { StarIcon, MessageSquareIcon, ShieldCheckIcon, GiftIcon, UsersIcon, XIcon, ChevronRightIcon } from 'lucide-react';
...
export const RatingTipModal: React.FC<RatingTipModalProps> = ({ isOpen, onClose, orderId, customerId }) => {
  const { 
    findOrder, 
    findUserById, 
    addServiceRating, 
  ...
  const handleSubmitRatings = async () => {
    if (!order || serviceRating === 0) {
      addNotification({ message: 'Vui lòng đánh giá chất lượng dịch vụ chung (ít nhất 1 sao).', type: 'warning' });
      return;
    }
    setIsSubmitting(true);

    addServiceRating({ orderId, customerId, rating: serviceRating as ServiceRating['rating'], comment: serviceComment });
    ...
    addNotification({ message: 'Cảm ơn bạn đã gửi đánh giá!', type: 'success' });
  ...
  const handleSubmitTips = () => {
    ...
    if (tipMade) {
      addNotification({ message: 'Cảm ơn bạn đã tip!', type: 'success' });
    }
    setIsSubmitting(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/70 dark:bg-slate-900/80 flex items-center justify-center p-4 z-[100] animate-fadeIn">
      <Card 
        ...
        className="w-full max-w-xl bg-bg-surface shadow-xl relative max-h-[90vh] flex flex-col"
        actions={<Button variant="ghost" onClick={onClose} className="absolute top-3 right-3 p-2"><XIcon size={20}/></Button>}
      >
        <div className="space-y-5 overflow-y-auto p-1 pb-4">
          {currentStep === 'rating' && (
            <>
              {/* Service Rating */}
