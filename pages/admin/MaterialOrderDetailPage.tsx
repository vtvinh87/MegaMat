...
const MaterialOrderDetailPage: React.FC = () => {
  ...
  const { materialOrders, addNotification } = useAppContext();
  ...
  const getStatusInfo = (status: MaterialOrder['status']): { textColor: string, bgColor: string, borderColor: string, text: string, icon: React.ReactNode } => {
    switch (status) {
      case 'Chờ duyệt':
        return { textColor: 'text-status-warning-text dark:text-amber-300', bgColor: 'bg-status-warning-bg dark:bg-amber-700/30', borderColor: 'border-status-warning dark:border-amber-600', text: 'Chờ duyệt', icon: <InfoIcon size={14} className="mr-1.5" /> };
      case 'Đã duyệt':
        return { textColor: 'text-status-success-text dark:text-emerald-300', bgColor: 'bg-status-success-bg dark:bg-emerald-700/30', borderColor: 'border-status-success dark:border-emerald-600', text: 'Đã duyệt', icon: <CheckCircle size={14} className="mr-1.5" /> };
      case 'Đã hủy':
        return { textColor: 'text-status-danger-text dark:text-rose-300', bgColor: 'bg-status-danger-bg dark:bg-rose-700/30', borderColor: 'border-status-danger dark:border-rose-600', text: 'Đã hủy', icon: <XCircle size={14} className="mr-1.5" /> };
      default:
    ...
    <div className="space-y-6">
      ...
      <Card title={`Chi tiết Đơn đặt NVL: ${order.id}`} titleClassName="!text-2xl !font-bold">
        <dl className="divide-y divide-border-base">
          ...
          <DetailItem label="Tổng tiền:" ddClassName="!text-xl !font-semibold text-brand-primary dark:!text-sky-400">
            ...
            <table className="min-w-full divide-y divide-border-base">
              ...
              <tbody className="bg-bg-surface divide-y divide-border-base">
                {order.items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-bg-surface-hover dark:hover:bg-slate-700/60 transition-colors">
