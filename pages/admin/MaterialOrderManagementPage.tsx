      ...
      <Card title="Tạo Đơn Đặt Nguyên Vật Liệu Mới" className="w-full max-w-2xl bg-bg-surface shadow-xl !border-border-base">
        <div className="space-y-5 max-h-[60vh] overflow-y-auto p-1">
          {items.map((item, index) => (
            <div key={index} className="p-4 border border-border-base rounded-lg bg-bg-subtle/30 dark:bg-slate-700/20 relative">
...
const MaterialOrderManagementPage: React.FC = () => {
  ...
  const getStatusClassAndText = (status: MaterialOrder['status']): { className: string, text: string } => {
    switch (status) {
      case 'Chờ duyệt': return { className: 'bg-status-warning-bg text-status-warning-text dark:text-amber-300 dark:bg-amber-700/40', text: 'Chờ duyệt' };
      case 'Đã duyệt': return { className: 'bg-status-success-bg text-status-success-text dark:text-emerald-300 dark:bg-emerald-700/40', text: 'Đã duyệt' };
      case 'Đã hủy': return { className: 'bg-status-danger-bg text-status-danger-text dark:text-rose-300 dark:bg-rose-700/40', text: 'Đã hủy' };
